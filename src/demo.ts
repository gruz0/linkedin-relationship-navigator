...
import { createWorkspace, type PersonAnnotation, type WorkspaceFile } from './workspace'
...
export function createDemoData(): ArchiveData {
  ...
  connections.push(
    {
      id: 'demo-unavailable-1',
      firstName: '',
      lastName: '',
      fullName: 'Unavailable profile',
      profileUrl: '',
      email: '',
      company: '',
      position: '',
      connectedOn: dateBefore(900),
      connectedOnRaw: '',
      roles: [],
...
export function createDemoWorkspace(data: ArchiveData): WorkspaceFile {
  ...
  for (const [index, person] of data.connections
    .filter((connection) => connection.isIdentifiable)
    .slice(0, 18)
    .entries()) {
    people[person.id] = {
