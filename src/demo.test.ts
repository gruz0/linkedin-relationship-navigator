...
describe('fictional demo workspace', () => {
  ...
  it('explains archive coverage, conversation counts, and privacy search limits', () => {
    ...
    expect(html).toContain('class="results-controls"')
    expect(html).toContain('aria-label="Result controls"')
    expect(html).toContain('aria-label="Connection filters"')
    expect(html).toContain('aria-label="Close filters"')
    expect(html).toContain('class="filter-mobile-footer"')
    expect(html).toContain('View <span aria-live="polite" aria-atomic="true">48 results</span>')
