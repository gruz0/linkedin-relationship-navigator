...
The six images in `screenshots` are reviewed captures from the repository owner’s real archive with Privacy mode enabled. They are deliberately not regenerated by the public script because the source archive is private. Do not replace them with unreviewed personal-data captures.
